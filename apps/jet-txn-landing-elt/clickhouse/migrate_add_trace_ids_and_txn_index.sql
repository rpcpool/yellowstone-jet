...
-- Not affected (left alone): mv_landed_transaction_slot_latency_1m (refreshable) and
-- landed_transaction_slot_latency (plain view, pre-existing).
--
-- `chain_entry_staging`, `chain_entry_with_txn_count_bounds` and `landed_transaction_entry_pos`
-- are brand new objects with no prior data -- nothing to migrate, but they DO need to exist
-- before step 4 recreates mv_landed_transaction_entry_pos, which reads from the view (itself
-- built on the table). Step 2 below creates all three, verbatim from schema.sql.
--
...
-- ============================================================================
-- 2. Create the brand new objects that mv_landed_transaction_entry_pos (recreated in step 5)
--    depends on. No prior data to migrate -- these didn't exist before this schema version --
--    but they must exist before that MV is created, so this can't be left as a "run
--    schema.sql yourself" footnote. Verbatim from schema.sql.
-- ============================================================================

CREATE TABLE IF NOT EXISTS jet.chain_entry_staging
(
    slot UInt64,
    entry_index UInt64,
...
CREATE OR REPLACE VIEW jet.chain_entry_with_txn_count_bounds AS
SELECT
    slot,
    entry_index,
...
FROM jet.chain_entry_staging;

CREATE TABLE IF NOT EXISTS jet.landed_transaction_entry_pos (
    signature String,
    slot UInt64,
    entry_index UInt64,
    txn_index UInt64,
...
-- ============================================================================
-- 3. Drop the materialized views that depend on any of the four tables from step 1 (as source
--    or target) -- see the header comment for why this has to happen before the rename swap.
--    The pipeline stops flowing between this point and step 5; keep that window short.
-- ============================================================================
...
-- ============================================================================
-- 4. Atomically swap each table in for its `_new` replacement. Each RENAME TABLE is one
...
-- ============================================================================
-- 5. Recreate the three materialized views dropped in step 3, verbatim from schema.sql --
...
-- ============================================================================
-- 6. Once you've verified row counts / spot-checked the swapped-in tables, drop the `_bak`
