//!
//! Generates random `TpuSenderResponse` values and feeds them through `TxnTraceShaper` +
//! `HttpNdjsonDrain`, so you can visually verify what actually gets HTTP POSTed by the
//! trace-drain pipeline.
//!
...
use {
    ...
    yellowstone_jet::{
        config::load_config,
        http_ndjson_drain::{HttpNdjsonDrain, HttpNdjsonDrainConfig},
        setup_tracing,
        transactions::JetTxnInfo,
        txn_trace_drain::{SolanaClientResolver, TxnTraceShaper},
    },
...
struct Config {
    ...
    ///
    /// TxDrop responses are never generated by this tool (only TxSent/TxFailed) -- not a
    /// limitation of the drain itself, which handles TxDrop fine, just not exercised here.
...
async fn main() -> anyhow::Result<()> {
    ...
    let txn_trace_entries = TxnTraceShaper::new(source, NoopSolanaClientResolver, None);
    let drain = HttpNdjsonDrain::with_config(txn_trace_entries, config.drain);
    drain.await?;
