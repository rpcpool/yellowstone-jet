...
pub struct TxnTraceEntry<'a> {
    ...
    pub remote_peer_identity: Option<Pubkey>,
...
fn get_txn_info<'a>(txn_response: &TpuSenderResponse) -> Option<OneOrMany<&Info>> {
    match txn_response {
        TpuSenderResponse::TxSent(tx_sent) => tx_sent
            .info
            .as_ref()
            .and_then(|info| info.downcast_ref::<Info>())
            .map(OneOrMany::One),
...
fn into_txn_trace_entry<'resp, 'info>(
    ...
    one_or_many: OneOrMany<&'info Info>,
    ...
    'resp: 'info,
{
    match txn_response {
        TpuSenderResponse::TxSent(tx_sent) => one_or_many.map(|info| TxnTraceEntry {
            ...
            state: TxnState::Sent,
            error_msg: None,
            remote_peer_identity: Some(tx_sent.remote_peer_identity),
        ...
        TpuSenderResponse::TxFailed(tx_failed) => one_or_many.map(|info| TxnTraceEntry {
            ...
            remote_peer_identity: Some(tx_failed.remote_peer_identity),
        ...
        TpuSenderResponse::TxDrop(tx_drop) => {
            let _ = tx_drop
                ...
                .map(|((_dropped_tx, _attempt), info)| TxnTraceEntry {
                    ...
                    state: TxnState::Drop,
                    error_msg: None,
                    remote_peer_identity: Some(tx_drop.remote_peer_identity),
...
impl<St> HttpTxnTraceDrain<St> {
    ...
    fn spawn_send_payload(&mut self) -> Result<(), HttpTxnTraceDrainError> {
        ...
        if let Some(payload) = self.pending_ndjson_payloads.pop_front() {
            ...
            let request = request_builder.body(payload).build()?;
...
mod tests {
    ...
    fn collect_signatures(one_or_many: &OneOrMany<&Info>) -> Vec<Signature> {
    ...
    async fn buffer_txn_response_appends_one_ndjson_line() {
        ...
        assert_eq!(parsed["signature"], serde_json::to_value(sig).unwrap());
    ...
    fn future_poll_sends_buffered_entries_to_a_real_http_server() {
        ...
        match result_rx.recv_timeout(std::time::Duration::from_secs(5)) {
            Ok((sig1, sig2, received)) => {
                ...
                assert!(received.contains(&serde_json::to_value(sig1).unwrap()));
                assert!(received.contains(&serde_json::to_value(sig2).unwrap()));
            }
            Err(_) => panic!(
