use {
    ...
    }, solana_clock::Slot, solana_epoch_schedule::EpochSchedule, solana_pubkey::Pubkey, std::{collections::HashMap, sync::Arc, time::Duration}, tokio::sync::{broadcast, mpsc, Mutex, RwLock}, yellowstone_jet::{
...
async fn test_slot_update() {
    ...
    let cluster_slot = cluster_tpu.latest_seen_slot().await;
