...
//!     /// If you don't care about responses, drop the `response` receiver to prevent memory leaks    
...
//!
//! ```sh
//! cargo run --bin test-tpu-send -- --rpc $RPC_ENDPOINT --grpc $GRPC_ENDPOINT --x-token $GRPC_X_TOKEN --recipient <RECIPIENT_PUBKEY>
//! ```
//!
pub mod config;
pub mod core;
