...
pub struct RpcSendTransactionConfigWithPolicies {
    #[serde(flatten)]
    pub config: RpcSendTransactionConfig,
    pub policies: Vec<Pubkey>,
}

impl RpcSendTransactionConfigWithPolicies {
    pub fn new(config: Option<RpcSendTransactionConfig>, policies: Option<Vec<String>>) -> Self {
...
impl TransactionPayload {
    ...
    pub fn create(
        ...
        config: RpcSendTransactionConfigWithPolicies,
        use_legacy: bool,
    ...
    pub fn to_legacy(
        transaction: &VersionedTransaction,
        config_with_policies: &RpcSendTransactionConfigWithPolicies,
    ) -> Result<Self, PayloadError> {
        let encoding = config_with_policies
            .config
            .encoding
        ...
        Ok(Self::Legacy(LegacyPayload {
            transaction: tx_str,
            config: config_with_policies.config,
...
impl TryFrom<(&VersionedTransaction, RpcSendTransactionConfigWithPolicies)> for TransactionPayload {
    type Error = PayloadError;

    fn try_from(
        (transaction, config_with_policies): (
            &VersionedTransaction,
            RpcSendTransactionConfigWithPolicies,
        ),
    ) -> Result<Self, Self::Error> {
        // Check encoding first to fail early if it's invalid
        if let Some(encoding) = config_with_policies.config.encoding {
            match encoding {
        ...
        // Create new payload format with policies supported
        Ok(Self::New(TransactionWrapper {
            transaction: tx_bytes,
            config: Some(TransactionConfig {
                max_retries: config_with_policies.config.max_retries.map(|r| r as u32),
                blocklist_pdas: config_with_policies
                    .policies
                    .iter()
                    .map(|p| p.to_string())
                    .collect(),
                skip_preflight: config_with_policies.config.skip_preflight,
                skip_sanitize: config_with_policies.config.skip_sanitize,
            }),
            timestamp: Some(ms_since_epoch()),
        }))
    }
...
impl TransactionDecoder {
    pub fn decode(
        ...
        (
            VersionedTransaction,
            Option<RpcSendTransactionConfigWithPolicies>,
        ),
        PayloadError,
    > {
        match payload {
            TransactionPayload::Legacy(legacy) => {
                ...
                Ok((
                    tx,
                    Some(RpcSendTransactionConfigWithPolicies::new(
                        Some(legacy.config),
                        None,
                    )),
                ))
...
impl TryFrom<&TransactionConfig> for RpcSendTransactionConfigWithPolicies {
    type Error = PayloadError;

    fn try_from(proto_config: &TransactionConfig) -> Result<Self, Self::Error> {
        Ok(RpcSendTransactionConfigWithPolicies::new(
...
mod tests {
    ...
    fn test_legacy_format() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let config = RpcSendTransactionConfigWithPolicies::new(
            Some(RpcSendTransactionConfig {
        ...
        let config_with_policies = decoded_config.unwrap();

        assert_eq!(decoded_tx.signatures, tx.signatures);
        assert_eq!(
            config_with_policies.config.skip_preflight,
    ...
    fn test_new_format_features() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let config_with_policies = config.unwrap();
        assert_eq!(config_with_policies.config.max_retries, Some(5));
        Ok(())
    }
    ...
    fn test_new_format() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(config_with_policies.config.max_retries, Some(5));
        assert!(config_with_policies.config.skip_preflight);
        assert_eq!(decoded_tx.signatures, tx.signatures);
        Ok(())
    }
    ...
    fn test_config_conversion() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let rpc_config_result: Result<RpcSendTransactionConfigWithPolicies, _> =
        ...
        assert_eq!(rpc_config.policies.len(), 1); // Policies are not validated at this stage
        Ok(())
    }
    ...
    fn test_config_invalid_pubkey_conversion() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let rpc_config = rpc_config_result?;
        assert_eq!(rpc_config.policies.len(), 1);
        Ok(())
    }
    ...
    fn test_transaction_config_sanitize_flags() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let config_with_policies = decoded_config.unwrap();
        assert!(config_with_policies.config.skip_preflight);
        assert!(config_with_policies.config.skip_sanitize);
        Ok(())
    }
    ...
    fn test_config_preservation_through_conversion() -> Result<(), Box<dyn std::error::Error>> {
        let tx = VersionedTransaction::default();
        let original_config = RpcSendTransactionConfigWithPolicies::new(
            Some(RpcSendTransactionConfig {
                skip_preflight: true,
                skip_sanitize: false,
                max_retries: Some(3),
        ...
        let config_with_policies = decoded_config.unwrap();
        assert_eq!(
            config_with_policies.config.max_retries,
        ...
        assert_eq!(
            config_with_policies.config.skip_sanitize,
    ...
    fn test_empty_config_conversion() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(rpc_config.policies.len(), 0);
        Ok(())
    }

    #[test]
    fn test_policies_conversion() -> Result<(), Box<dyn std::error::Error>> {
        let tx = VersionedTransaction::default();
        let config = RpcSendTransactionConfigWithPolicies::new(
        ...
        assert!(decoded_config.is_some());
        let decoded_policies = decoded_config.unwrap().policies;
        assert_eq!(decoded_policies.len(), 1); // Policies are not preserved through decode
        Ok(())
    }

    #[test]
    fn test_policies_pubkeys_conversion() -> Result<(), Box<dyn std::error::Error>> {
        let config = RpcSendTransactionConfigWithPolicies::new(
            None,
            Some(vec![
                "11111111111111111111111111111111".to_string(), // Valid pubkey
        ...
        assert_eq!(config.policies.len(), 1); // Only valid pubkey is included
